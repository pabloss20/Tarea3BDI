        ...
        public IActionResult FiltroNombre([FromBody] FiltroNombre filtroNombre)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(_configuration.GetConnectionString("connection").ToString()))
                {
                    ...
                    SqlCommand command = new SqlCommand("MostrarNombreDeArticulos", connection);
                    command.CommandType = CommandType.StoredProcedure;

                    // Configura los parámetros de entrada (BD, Modelo)
                    command.Parameters.AddWithValue("@inUsername", "pablo");
                    ...
                    SqlParameter autenticadoParam = new SqlParameter("@outRetorno", SqlDbType.Bit);
